...
def random_nth_price_auction(observation):
    payment_lst = []

    for action in range(21):  # 遍历所有动作
        randn_payment = compute_randn_payment(action, observation)

        if randn_payment is None:
            # 向右方的随机值
            return random.choice([3, 7, 8, 11])
            # 移动动作的随机值
            # return random.randint(0, 12)

        payment_lst.append([randn_payment, action])

    payment_lst.sort(key=lambda x: x[0])
    rand_sum = int(21 * 0.1)
    best_action = random.choice(payment_lst[:rand_sum])[1]
    return best_action


def compute_randn_payment(action, current_obs):
    impact = compute_impact(action, current_obs)
    if impact is None:
        return None
    else:
        # 计算其他红方智能体选择该动作的估值，即支付
        randn_payment = -impact  # 注意这里取负值
        return randn_payment
